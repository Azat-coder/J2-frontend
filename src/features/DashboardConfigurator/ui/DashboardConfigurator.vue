<template>
  ...
  onMounted(() => {
      ...
      nextTick(() => {
          console.log('Инициализация данных дерева:', treeTableValue.value); // Отладка
          syncSelectedFromDashboard(dashboardStore.dashboardLayout);
          processItems(dashboardStore.dashboardLayout);
      });
  });
  
  // Логика обработки чекбоксов и элементов дашборда
  watch(selectedTreeTableValue, (value) => {
      ...
      // Добавляем или удаляем элементы в layout
